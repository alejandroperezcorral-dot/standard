-- STDTEX Cost Model 001 canonical dormant production migration
-- Generated for review only. DO NOT execute without explicit approval.
-- Production baseline: no Costing objects exist.
-- Contract: creates final dormant Costing state only; no company activation, no active configs.
-- Expected active Costing configs after successful execution: 0.
begin;

-- Stop if any Costing object already exists. This migration is only for the dormant no-Costing production baseline.
do $$
begin
  if to_regclass('public.cost_models') is not null
     or to_regclass('public.company_costing_settings') is not null
     or to_regclass('public.cost_config_versions') is not null
     or to_regclass('public.cost_config_overrides') is not null
     or to_regclass('public.cost_additional_components') is not null then
    raise exception 'COSTING_OBJECTS_ALREADY_EXIST';
  end if;
end $$;
...
-- Final semantic validation hash protection and activation guard.
...
-- Dormant-state invariant: no company is enabled for STDTEX costing and no active configs exist.
do $$
declare
...
begin
  ...
  if v_active_configs <> 0 then
    raise exception 'COSTING_ACTIVE_CONFIGS_NOT_DORMANT: %', v_active_configs;
  end if;
  if v_enabled_companies <> 0 then
    raise exception 'COSTING_COMPANIES_NOT_DORMANT: %', v_enabled_companies;
  end if;
end $$;
